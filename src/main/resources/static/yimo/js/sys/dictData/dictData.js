						...
						},
						{
							...
							formatter : function(value, row, index) {
								var e = '<a class="btn btn-primary btn-sm ' + s_edit_h
										...
										+ row.dictType + '\',\''+row.dictNo+'\')"><i class="fa fa-edit"></i></a> ';
								var d = '<a class="btn btn-warning btn-sm ' + s_remove_h
										+ '" href="#" title="删除"  mce_href="#" onclick="remove(\''
										+ row.dictType + '\',\''+row.dictNo+'\')"><i class="fa fa-remove"></i></a> ';
								return e + d;
							}
						} ]
			});
...
function add() {
	layer.open({
		...
		area : [ '800px', '520px' ],
		content : prefix + '/add/' + dictType // iframe的url
	});
}
// 编辑
function edit(id,dictNo) {
	layer.open({
		type : 2,
		title : '编辑',
		maxmin : true,
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/edit/' + id + '/' + dictNo// iframe的url
	});
}
// 删除
function remove(id,dictNo) {
